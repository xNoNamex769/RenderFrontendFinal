...
const CatalogoDisponible = () => {
  ...
  const totalDisponibles = catalogo.reduce(
    (sum, el) => sum + el.CantidadDisponible,
    0
  );
  ...
  return (
    <div className="cat-disp-wrapper">
      ...
      <div className="cat-disp-header">
        <h1 className="cat-disp-title">
          <FaBox /> Catálogo de Elementos
        </h1>
        <p className="cat-disp-subtitle">
          Explora los elementos disponibles para alquiler
        </p>
      </div>
      ...
      <div className="cat-disp-stats">
        <div className="cat-disp-stat-card">
          <div className="cat-disp-stat-icon">
            <FaClipboardList />
          </div>
      ...
      {catalogo.length === 0 ? (
        <div className="cat-disp-empty">
          <div className="cat-disp-empty-icon">
            <FaInbox />
          </div>
            ...
            <div key={el.IdElemento} className="cat-disp-card">
              <div className="cat-disp-img-wrapper">
                ...
                <div className="cat-disp-badge-wrapper">
                  <span
                    className={`cat-disp-badge ${
                      el.CantidadDisponible > 0 ? "disponible" : "agotado"
                    }`}
                  >
                    {el.CantidadDisponible > 0 ? (
                      <>
                        <FaCheck /> Disponible
                      </>
                    ) : (
              ...
              <div className="cat-disp-card-body">
                ...
                <div className="cat-disp-stock">
                  <span className="cat-disp-stock-icon">
                    <FaBox />
                  </span>
                  <span className="cat-disp-stock-text">
                    {el.CantidadDisponible} unidades
                  </span>
                </div>
                <button
                  onClick={() => setQrActivo(el)}
                  className="cat-disp-btn"
                >
      ...
      {qrActivo && (
        <div className="cat-disp-modal-overlay" onClick={cerrarModal}>
          <div
            className="cat-disp-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            <button className="cat-disp-modal-close" onClick={cerrarModal}>
              <FaTimes />
            </button>
            <h3 className="cat-disp-modal-title">{qrActivo.Nombre}</h3>
            <QRGeneradorPrestamo
              IdElemento={qrActivo.IdElemento}
              nombreElemento={qrActivo.Nombre}
            />
          </div>
        </div>
      )}
