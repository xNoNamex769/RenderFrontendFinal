...
const sitiosReferencia = [
  {
    ...
    nombre: "Recreacion",
    categoria: "entrada",
    descripcion: "Cancha de deportes",
    coordenadas: [2.4832482, -76.5617734],
    icono: "🚪",
    color: "#39A900",
    servicios: ["Actividades", "Eventos", "Formacion"],
    imagen: "/img/coliseo2.jpeg",
  },
  {
    id: 2,
    nombre: "Biblioteca",
  ...
  },
  {
    ...
    descripcion: "Eventos Actividades y más",
    coordenadas: [2.4833, -76.5615],
    icono: "⚽",
    color: "#F44336",
    servicios: ["Actividades", "Eventos", "Charlas"],
    imagen: "/img/coliseo.jpeg",
  },
...
function MapaReferencia() {
  ...
  console.log("Centro por defecto:", centroDefault);
  ...
  useEffect(() => {
    const cargarDatosAPI = async () => {
      try {
        ...
        // Cargar eventos
        const resEventos = await axios.get(
          "https://render-hhyo.onrender.com/api/evento/publicos"
        );
        ...
        // Cargar lúdicas
        const resLudicas = await axios.get(
          "https://render-hhyo.onrender.com/api/ludica"
        );
        const soloLudicas = resLudicas.data.filter(
          (item) =>
  ...
  const eventosComoSitios = eventosAPI.map((evento, idx) => ({
    ...
    imagen:
      evento.PlanificacionEvento?.ImagenUrl ||
  ...
  const sitiosFiltrados = todosSitios.filter((sitio) => {
    const coincideCategoria =
      filtroCategoria === "todos" || sitio.categoria === filtroCategoria;
    ...
    <div className="mapa-referencia-container">
      ...
      <div className="mapa-header">
        ...
        <p className="mapa-subtitulo">
          Encuentra lugares importantes cerca de ti
        </p>
      ...
      {!cargando && mostrarInfo && (
        <div className="mapa-info-banner">
          <FaInfoCircle />
          <span>
            📍 {sitiosReferencia.length} lugares | 🎭 {eventosAPI.length}{" "}
            eventos | 📚 {actividadesAPI.length} actividades | ⚽{" "}
      ...
      <div className="mapa-controles">
        <div className="mapa-busqueda-container">
          ...
          {ubicacionUsuario && (
            <button
              ...
              onClick={() =>
                setSitioSeleccionado({ coordenadas: ubicacionUsuario })
              }
        ...
        {(busqueda || filtroCategoria !== "todos") && (
          <div className="mapa-resultados">
            <span>
              🔍 {sitiosFiltrados.length} resultado
              {sitiosFiltrados.length !== 1 ? "s" : ""} encontrado
              {sitiosFiltrados.length !== 1 ? "s" : ""}
            </span>
            {(busqueda || filtroCategoria !== "todos") && (
              <button
              ...
              >
                <Popup maxWidth={350}>
                  <div className="popup-content">
                    ...
                    <h3>
                      {sitio.icono} {sitio.nombre}
                    </h3>
                    {sitio.tipo && (
                      <div className="popup-tipo-badge">
                        {sitio.tipo === "evento" && (
                          <span className="badge-api evento">
                            🎭 Evento en vivo
                          </span>
                        )}
                        {sitio.tipo === "actividad" && (
                          <span className="badge-api actividad">
                            📚 Actividad programada
                          </span>
                        )}
                        {sitio.tipo === "ludica" && (
                          <span className="badge-api ludica">
                            ⚽ Lúdica disponible
                          </span>
                        )}
                      </div>
                    ...
                    <div className="popup-servicios">
                      <strong>
                        {sitio.tituloDetalles || "Servicios disponibles"}:
                      </strong>
                      <ul>
                        {(sitio.detalles || sitio.servicios || []).map(
                          (item, idx) => (
                            <li key={idx}>✓ {item}</li>
                          )
                        )}
        ...
        <div className="mapa-lista">
          ...
          <div className="lista-sitios">
            {sitiosFiltrados.map((sitio) => (
              <div
                key={sitio.id}
                className={`sitio-card ${
                  sitioSeleccionado?.id === sitio.id ? "seleccionado" : ""
                }`}
                onClick={() => setSitioSeleccionado(sitio)}
              >
                <div
                  className="sitio-icono"
                  style={{ backgroundColor: sitio.color }}
                >
                  {sitio.icono}
                </div>
